  ...
  return (
    <div className="flex-1 min-w-0 mb-2 lg:mb-0 h-full">
      <Card className="w-full h-full min-h-[420px] flex flex-col shadow-2xl hover:-translate-y-1 transition-all duration-200 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800">
        <CardHeader className="flex items-center justify-between px-2 sm:px-4 pt-4 pb-2">
        ...
        <CardContent className="flex-1 flex flex-col justify-between px-2 sm:px-4 pb-4">
          <ul className="space-y-3 flex-1 flex flex-col justify-center">
            {team.slice(0, -1).map((member) => (
              <li
                key={member.name}
                className="flex items-center justify-start gap-3 sm:gap-4 cursor-pointer hover:bg-accent rounded transition px-2 py-2 relative"
      ...
      {chatMember && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg min-w-[320px] max-w-xs w-full border">
